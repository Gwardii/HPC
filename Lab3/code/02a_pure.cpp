...
void BM_vec_quad(benchmark::State& state)
{
    ...
    const auto        src_alloc  = makeRandomAlignedArray< double >(size);
    auto              dest_alloc = makeRandomAlignedArray< double >(size);

    const auto src  = std::assume_aligned< 64 >(src_alloc.get());
    auto       dest = std::assume_aligned< 64 >(dest_alloc.get());
    for (auto _ : state)
    {
        for (std::ptrdiff_t i = 0; i < size; ++i)
            dest[i] = src[i] * src[i] + src[i] * 3.5 + 1.;
